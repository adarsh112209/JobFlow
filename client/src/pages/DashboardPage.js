    ...
    const DashboardPage = () => {
      ...
      const refreshApplications = useCallback(async () => {
        ...
        try {
          ...
          const { data } = await axios.get('/api/users/applications', config); // Assuming a new route for apps
      ...
      const handleEditApplication = async (formData) => {
        try {
          const config = { headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${userInfo.token}` } };
          await axios.put(`/api/applications/${editingApplication._id}`, formData, config);
          closeModal();
          refreshApplications();
      ...
      const confirmDeleteHandler = async () => {
        try {
          const config = { headers: { Authorization: `Bearer ${userInfo.token}` } };
          await axios.delete(`/api/applications/${editingApplication._id}`, config);
      ...
      const handleScanGmail = async () => {
        ...
        try {
          ...
          const { data } = await axios.post('/api/gmail/analyze', {}, config);
